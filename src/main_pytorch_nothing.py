...
This model is running when no ELMo is selected
...
>>time python main_pytorch_nothing.py -m cnn -elmo False -elmo_level 2 -elmo_train False -e 4 -over False
...
result_path = 'final_nothing_after'
...
save_path = 'final_wtf'  # model save path
...
def train(config):
    ...
    # TODO: for oversampling only
    # train_data = TensorDataset(torch.LongTensor(corpus.x_train_text), torch.LongTensor(corpus.x_train_text),
    #                            torch.LongTensor(corpus.y_train))
    # test_data = TensorDataset(torch.LongTensor(corpus.x_test_text), torch.LongTensor(corpus.x_test_text),
    ...
    test_acc, test_prf, y_true,y_pred = test(model, test_data, config.model_file)


    return test_acc, test_prf, y_true,y_pred
...
def test(model, test_data, model_file):
    ...
    test_prf = metrics.precision_recall_fscore_support(y_true, y_pred, average='macro')
...
if __name__ == '__main__':
    ...
    for i,test_file in enumerate(test_file_list):
        ...
        score, prf,y_true,y_pred = train(config)
        ...
        print(config.abbre, score,prf[2])
    ...
    avg_re = sum(recall) / sum(recall)
    print('Average, %0.4f,%0.4f,%0.4f,%0.4f' % (avg_acc, avg_pr,avg_re,avg_f1))
