...
def clean_str(s):
    ...
    # s = [t for t in s.split(' ') if t not in stopwords]
...
def read_test(test_path,abbre):
    ...
    for i in range(len(xl)):
        ...
        if xl[0][i] == 'Y' or (pd.isnull(xl[0][i]) and str(xl[1][i]).startswith('http')):
...
class Corpus(object):
    ...
    def __init__(self, file_path, test_path, abbre, max_length=50, vocab_size=8000):
        ...
        #ID shape for CNN... torch.Size([3, 172, 50])
        #Text shape for LSTM... (3, 200)
...
#
# Corpus(file_path, config.test_path, config.abbre, config.seq_length, config.vocab_size)