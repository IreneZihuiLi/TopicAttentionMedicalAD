...
class CNNClassifier(nn.Module):
    ...
    def __init__(self, config):
        ...
        if self.elmo:
            ...
            self.embedding = nn.Embedding(self.vocab_size, int(self.embedding_dim/2)) # concate
    ...
    def forward(self, inputs,inputs_ids,batch_size=None):
        ...
        if self.elmo:
            ...
            time_step_elmo = sents.size()[1]  # we should slice the embedded
            embedded = self.embedding(inputs)

            time_step_emb = embedded.size()[1]
            # print ('Time Step ',time_step_elmo,time_step_emb)

            # keep smaller dimension
            if time_step_elmo < time_step_emb:
                sents = torch.cat((sents, embedded[:,:time_step_elmo]), 2)
            else:
                sents = torch.cat((sents[:, :time_step_emb], embedded), 2)
