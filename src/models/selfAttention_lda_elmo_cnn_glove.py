...
word2vec_path='/home/lily/zl379/data/GloVe/glove.6B.100d.txt'
...
class SelfAttention(nn.Module):
    def __init__(self, config):
        ...
        print('USE topic?...', config.use_topic)
        if self.use_elmo == 'True':
            ...
            dict_glove = pd.read_csv(filepath_or_buffer=self.word2vec_path, header=None, sep=" ",
            ...
            self.embedding = nn.Embedding(num_embeddings=dict_len, embedding_dim=embed_size).from_pretrained(
                self.dict_glove, freeze=True)

            content = pd.read_csv(filepath_or_buffer=self.word2vec_path, header=None, sep=" ", quoting=csv.QUOTE_NONE,
                                  usecols=[0]).values
            ...
            dict_glove = pd.read_csv(filepath_or_buffer=self.word2vec_path, header=None, sep=" ", quoting=csv.QUOTE_NONE).values[:,1:]
            ...
            unknown_word = np.zeros((1, embed_size))
            self.dict_glove = torch.from_numpy(np.concatenate([unknown_word, dict_glove], axis=0).astype(np.float))
            self.embedding = nn.Embedding(num_embeddings=dict_len, embedding_dim=embed_size).from_pretrained(self.dict_glove,freeze=False).cuda()
            ...
            if config.use_topic:
                ...
                content = pd.read_csv(filepath_or_buffer=self.word2vec_path, header=None, sep=" ", quoting=csv.QUOTE_NONE,
                                      usecols=[0]).values
                words = [word[0] for word in content]
                print('W2v Loaded...', len(words))
                self.word_to_id = dict(zip(words, range(len(words))))

                self.load_topic(self.dict_glove, dict_len, self.lda_embed_size)
        ...
        self.fc_layer_topic = nn.Linear(self.attention_dim * 2 * self.hidden_size + self.lda_embed_size,128)
    ...
    def load_topic(self,dict_glove,dict_len, embed_size):
        ...
        print('Loaded topics...', self.topic_words_embed.size()) # torch.Size([50, 100, 100]) (n_topics, n_top_words, dim)
    ...
    def topic_conv2d(self):
        ...
        # self.conv_fc = nn.Liner(10*10,1)


    def get_topic_matrix(self):
        hidden = torch.unsqueeze(self.topic_words_embed,1).type(torch.FloatTensor)
        hidden = self.conv(hidden.cuda())
        hidden = self.pool(hidden)
        output_list = []
        for i in range(50):
            index = torch.tensor([i]).cuda()
            selected = torch.index_select(hidden, 0, index)
            selected = F.tanh(selected.view(-1, 10 * 10))
            output_list.append(selected)

        new_shape = torch.stack(output_list, 0).size()[1]*torch.stack(output_list, 0).size()[2]
        self.topic_matrix = torch.stack(output_list, 0).view(50,new_shape) # shape (50,100)
