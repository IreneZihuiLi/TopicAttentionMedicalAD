...
class SelfAttention(nn.Module):
    def __init__(self, config):
        ...
        # elmo parameters
        self.use_elmo = config.elmo == 'True'
        self.requires_grad = True if config.elmo_train == 'True' else False

        # lda parameters
        self.lda_path = config.lda_path
        self.use_topic = self.use_elmo = True if config.use_topic == 'True' else False
        ...
        # for topic attention
        self.W_topic = nn.Linear(32 * 2 * self.hidden_size, self.lda_embed_size) # 6400, 100


        self.fc_layer = nn.Linear(32 * 2 * self.hidden_size, 128)


        self.fc_layer_topic = nn.Linear(32 * 2 * self.hidden_size + self.hidden_size,128)
    ...
    def topic_attention(self,content):
        ...
        # Hidden Matrix...torch.Size([10, 32, 200])
    ...
    def forward(self, inputs, input_sentences, batch_size=None):
        ...
        # hidden_matrix...torch.Size([10, 32, 200])



        # print('BMM...', topic_weight_matrix.size(), hidden_matrix.size())
